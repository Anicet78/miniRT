...
{
	t_aabb			aabb;
	const t_vec		half_axis = vmul(cyl->axis, cyl->h * 0.5f);
	const t_point	base1 = vsub(cyl->pos, half_axis);
	const t_point	base2 = vadd(cyl->pos, half_axis);
	t_vec			ext;

	ext.x = cyl->r * sqrtf(1.0f - cyl->axis.x * cyl->axis.x);
	ext.y = cyl->r * sqrtf(1.0f - cyl->axis.y * cyl->axis.y);
	ext.z = cyl->r * sqrtf(1.0f - cyl->axis.z * cyl->axis.z);
...
static t_aabb	get_cone_aabb(t_cone *cone)
{
	...
	const t_point	base = vadd(cone->pos, vmul(cone->axis, cone->h));

	ext.x = cone->r * sqrtf(1.0f - cone->axis.x * cone->axis.x);
	ext.y = cone->r * sqrtf(1.0f - cone->axis.y * cone->axis.y);
	ext.z = cone->r * sqrtf(1.0f - cone->axis.z * cone->axis.z);
